...
const DIM_LONG = {
  ...
  U: 'Use', US: 'User Satisfaction',
  II: 'Individual Impact', OI: 'Organizational Impact'
};
const DIM_BLURB = {
  ...
  II: 'How does it change the clinician\u2019s work and well-being?',
  OI: 'What outcomes does it produce for the organization and its patients?'
};

// Order matters: check longer codes first so "US" is not shadowed by "U",
// and "SerQ" is not shadowed by "SQ".
// Order matters: longer codes BEFORE their prefixes so substring matches resolve correctly:
// 'SerQ' before 'SQ', 'US' before 'U', 'II' before 'I' (none here), 'OI' before 'O' (none here).
const DIM_CODES_ORDERED = ['SerQ','US','SQ','IQ','II','OI','U'];
...
function DimTag({ code, style }) {
  if (!code) return null;
  return <span className={`dim-tag dim-${code}`} style={style}>{code}</span>;
}
...
const CONVERGENCE = {
  ...
  'CM-16':{c:'converge', r:'Across the 3 papers measuring this formally, the signal is convergent: perceived reliability gates adoption. Clinicians who doubt output accuracy either reject the tool outright or over-verify every note, both manifest as adoption failure rather than satisfaction decline.'},
