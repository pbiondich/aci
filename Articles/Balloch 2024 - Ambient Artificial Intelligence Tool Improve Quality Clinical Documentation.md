# Article Review

---

## Article Title
Use of an ambient artificial intelligence tool to improve quality of clinical documentation

## Author(s)
Balloch, Jasmine; Sridharan, Shankar; Oldham, Geralyn; Wray, Jo; Gough, Paul; Robinson, Robert; Sebire, Neil J.; Khalil, Saleh; Asgari, Elham; Tan, Christopher; Taylor, Andrew; Pimenta, Dominic

## DOI
[10.1016/j.fhj.2024.100157](https://doi.org/10.1016/j.fhj.2024.100157)

## Journal / Source
Future Healthcare Journal | Vol. 11, Issue 3 | September 2024 | 100157. Open Access. Future Healthcare Journal (Royal College of Physicians). Simulation study.

## Ambient Product(s) Evaluated
Ambient AI tool (unnamed in abstract — likely DAX or similar). Used in simulated outpatient consultations with actors and clinicians.

## Clinical Setting
Simulation study — outpatient consultations simulated with actors and clinicians. Documentation assessed using Sheffield Assessment Instrument for Letters (SAIL). Clinician experience measured through questionnaires and NASA Task Load Index. No real patients.

## Evidence Level
Observational (simulation study)

## Data Source(s)
Simulated consultations | SAIL for documentation quality | Questionnaires and NASA Task Load Index for clinician experience

## Article Summary
This simulation study evaluated an ambient AI tool's impact on clinical documentation quality and clinician experience. AI-produced documentation achieved higher SAIL scores than standard EHR practices. Consultations were 26.3% shorter on average without impacting patient interaction time. Clinicians reported an enhanced experience and reduced task load. The study concludes that the AI tool significantly improved documentation quality and operational efficiency in simulated consultations. Clinicians recognized its potential to improve note-taking processes.

## Hypothesis
To assess the clinical utility of an ambient AI tool in enhancing consultation experience and the completion of clinical documentation.

## Study Measures

**D&M Dimensions reference:**
- **SQ** = System Quality | **IQ** = Information Quality | **SerQ** = Service Quality
- **U** = Use / Intention to Use | **US** = User Satisfaction | **NB** = Net Benefits

| Measure | Description | D&M Dimension(s) |
|---|---|---|
| SAIL Score | Sheffield Assessment Instrument for Letters for documentation quality. AI-produced documentation achieved higher SAIL scores. | [[IQ - Information Quality]] |
| Consultation Duration | AI consultations 26.3% shorter on average without impacting patient interaction time. | [[NB - Net Benefits]] |
| Clinician Experience | Enhanced experience and reduced task load reported by clinicians (NASA Task Load Index). | [[US - User Satisfaction]] |

---
*Reviewed by Iris | Source: Future Healthcare Journal 2024 | DOI: 10.1016/j.fhj.2024.100157 | Template v2.0 | Full text reviewed ✅*
*📍 Simulation study using Sheffield Assessment Instrument for Letters (SAIL) for documentation quality. AI achieved higher SAIL scores and reduced consultation duration by 26.3% without impacting patient interaction time.*
*📍 Clinicians reported enhanced experience and reduced task load. This is consistent with other simulation studies in the corpus (Biro 2025, Anderson 2025).*
*📍 UK-based study from Future Healthcare Journal (Royal College of Physicians). Adds geographic diversity.*
*📍 Note: Abstract-only content available — full text would provide specific SAIL scores and NASA Task Load Index details.*
